...
def main(on_progress=None) -> int:
    ...
    bar, so the price fetch reports progress the same way the screen does.
    """
    ...
    with Cache(args.db) as db:
        ...
        for i, (lei, listing) in enumerate(targets, 1):
            ...
            if args.source == "yahoo":
                ...
                # Never fall through to a default source: `else: csv` would
                # let a mistyped source silently read an empty CSV and
                # report "not in CSV" for every company without once
                # mentioning that the requested source was ignored.
