...
WHY THE SPLIT
-------------
Everything used to live in run_screen.main(): the company loop, the policy
decision, the terminal table and the file writing, interleaved across 250
lines. That works exactly once, for exactly one kind of output. A GUI
cannot reuse a print statement, and neither can a web page or a scheduled
job, so each new interface would have re-derived the same decisions in a
new place and the copies would have drifted.
...
class RunSummary:
    ...
    def check_text(self) -> dict[str, str]:
        ...
        Must come from a company where the check actually FIRED. An earlier
        version took the first message seen for each cell, and since most
        companies pass most checks, that was almost always the literal
        string "OK" -- so the run summary reported lines like
        "14 of 41 (34%) D103 OK", which says nothing at all.
        """
        out: dict[str, str] = {}
        for c in self.companies:
            for chk in c.checks:
...
def reinvestment_ratio(result: dict) -> float | None:
    ...
    OPPOSITE SIGNS ARE A CONTRADICTION, NOT A MAGNITUDE GAP. An earlier
    version took abs() of both before dividing, so a cash-flow estimate of
    -30% against a sales-to-capital estimate of +49% scored 1.63 -- a mild
    disagreement -- and passed the graded rule into the research queue.
