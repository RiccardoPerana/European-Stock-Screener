...
def stages(db: Path = None, params_path: Path = None,
    ...
    # design, so comparing mapped (150) against every listing (158) always
    # showed a phantom "8 to do".
    classifiable = max(listings - len(excluded), 0)

    # Two independent try blocks, not one: they used to share a block and
    # an exception from either wiped out BOTH counts, so a failure in the
    # mapping count (need_mapping) discarded an already-correct `ready` and
    # made the Dashboard falsely report nothing screenable.
    ready = 0
