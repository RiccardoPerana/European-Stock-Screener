...
WHY THIS EXISTS
---------------
The fiscal window was written out longhand in sixteen scripts. Moving it
into config.py is a mechanical edit across sixteen files, and mechanical
edits across many files are exactly where a careless string replace does
quiet damage -- earlier in this project one truncated a constant and left
`WIKIDATA_` behind, which only surfaced when the module failed to import.

So the invariants are asserted here rather than eyeballed:

  1. config.FISCAL_YEARS is still the window everything used to hardcode.
  2. No script has gone back to hardcoding it.
...
# The window every script used to hardcode. If the project rolls forward,
...
def test_years_default_resolves_to_the_window(script):
    ...
    years. Importing config in sixteen files is only safe if it yields
    what the literals used to.
    """
    path = find_script(script)
    if path is None:
        pytest.skip(f"{script} not present")
    ...
    #
    # An earlier version called m.main() and let it proceed. On a machine
    # with no database that returns immediately; on a real one it started
    # an actual 129-company screening run and hit the 60s timeout. The
    # same loop covers track.py, extraction_audit.py and fetch_prices.py,
    # so it was one argparse quirk away from writing to a portfolio or
    # spending API calls. A test must never do the thing it is testing.
    #
