...
Section 3.1 requires exactly five CONSECUTIVE years; the validation used to
check length and ascending order only, so a window with a gap in it (e.g.
skipping one year) passed silently and every multi-year calculation
downstream would have quietly used a non-consecutive window.
"""
