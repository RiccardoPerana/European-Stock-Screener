...
def test_blank_b63_raises_value_error_not_type_error():
    ...
    Run unconditionally, the B63 + B64 == 10 check would crash a blank B63
    (None) on `None + int` with a raw TypeError before the ValueError this
    function exists to raise was ever reached.
    """
    values = _valid_values()
    values["B63"] = None
    with pytest.raises(ValueError, match="Template conventions have drifted"):
