...
They used to share one try/except: an exception raised while computing
need_mapping (the industry-mapping count) reset an already-correctly-
...
def test_prices_fetched_today_is_not_stale_against_identity_over_a_weekend(
    ...
    wrong. Comparing identity's "now" against prices' "Friday" flagged
    prices as needing a re-run it had already just had, and the alert could
    never clear until markets reopened. ran_date (fetched_at) is the fix:
    it reflects when the fetch itself ran, which is also "now" here.
    """
