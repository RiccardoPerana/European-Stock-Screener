...
[![Python 3.10+](https://img.shields.io/badge/python-3.10%2B-6e7781?style=flat-square)](https://www.python.org/)
[![Stack: Excel · SQLite · GitHub Actions](https://img.shields.io/badge/stack-Excel%20%C2%B7%20SQLite%20%C2%B7%20GitHub%20Actions-6e7781?style=flat-square)](#technical-architecture-and-approach)
[![Screen](https://img.shields.io/github/actions/workflow/status/RiccardoPerana/European-Stock-Screener/screen.yml?branch=main&label=screen&style=flat-square)](https://github.com/RiccardoPerana/European-Stock-Screener/actions/workflows/screen.yml)
...
[![Live Preview](https://img.shields.io/badge/Live%20Preview-0969da?style=for-the-badge)](https://riccardoperana.github.io/European-Stock-Screener/)
[![Methodology](https://img.shields.io/badge/Methodology-0969da?style=for-the-badge)](https://riccardoperana.github.io/European-Stock-Screener/methodology.html)

</div>

<p align="center">
  <img src="screenshots/presentation.png"
       alt="The Results page: a research queue of undervalued companies ranked by upside, with the funnel showing how the valued universe narrowed to that queue."
       width="900">
</p>
<p align="center">
  <em>The output of a screening run — whatever the model found, not a curated list.</em>
</p>

The screener pulls financial statements from the ESEF (XBRL) filing archive,
prices from Yahoo Finance, and risk parameters from Damodaran's datasets and
the ECB; writes each company into an Excel valuation model; reads the verdict
back out; and tracks the resulting picks over time. Its core output is a
research queue of companies priced below their calculated fair value, ranked
by upside.
...
within what the data allows; the only thing that determines whether a
company gets flagged is whether its own published financial statements say
it is priced below its calculated fair value.

That universe is every company the data can support: those that have filed
five consecutive years of ESEF reports, narrowed to the ones with a primary
listing quoted in euros, a price and a share count, with banks, insurers
and property companies excluded — 129 companies in the September 2026
screen. See [Limitations](#limitations-and-future-improvements).
...
systematic screen of every eligible company possible for close to the first
time.

<p align="center">
  <img src="screenshots/chart.png"
       alt="European companies filing ESEF by fiscal year"
       width="900">
</p>
...
> it flags will outperform the broader market. The validation tooling
> described below (see
> [Correctness and validation](#correctness-and-validation))
> exists to confirm that the model computes what it claims to compute. The
> portfolio tracker is a transparent log of how the prices of flagged
> companies moved after they were flagged; it makes no claim about
> investment performance. Reported returns reflect price performance only
> and exclude dividends.

## Technical architecture and approach

Built with Python (openpyxl for the workbook, SQLite for the extracted
financials, a standard-library web server for the dashboard), an Excel
discounted-cash-flow model recalculated by Microsoft Excel or LibreOffice,
PyInstaller for the portable executable, and GitHub Actions and GitHub
Pages for the published preview.
...
separate, price-redacted version of the results is rebuilt
...
**[Live preview](https://riccardoperana.github.io/European-Stock-Screener/)** —
...
Run the tool yourself (see [Quick start and usage](#quick-start-and-usage) below) for the
full, private version — real € prices and figures included.
...
No prebuilt executable is published; it is built from a source checkout,
and the result then runs on any Windows machine without Python. To build
`StockScreen.exe` (again after any code or config change):
...
Double-clicking `StockScreen.exe` opens `http://127.0.0.1:8765/` in your
browser — the same dashboard as above. The first run still needs the
coverage/extract/identity/prices/shares pipeline stages run once (from the
Dashboard's "Run Everything" button) to populate real data, same as a fresh
checkout would.
...
- **No durable competitive advantage.** Terminal return on invested
  capital converges to the cost of capital, so the model cannot recognize
  a lasting competitive advantage: quality compounders read as expensive,
  and the undervalued list skews towards capital-intensive, low-multiple
  businesses.
- **Euro-quoted listings only.** The model works in euros and performs no
  currency conversion. A company whose primary listing is quoted in another
  currency is rejected rather than converted, so companies listed in
  Sweden, Denmark, Poland and other non-euro markets are outside the
  screen even when they file ESEF reports.
- **Financials excluded.** Banks, insurers and property companies are
  removed, since a discounted-cash-flow model built for operating
  companies does not apply to them.
- **A fixed five-year window.** Every company is valued on the same five
  fiscal years (currently 2021–2025), with margins normalized to the
  five-year median. A company without all five years of filings cannot be
  valued, and conditions that persist across most of the window, such as
  a multi-year commodity or energy-price cycle, are not normalized away by
  the median.
...
The regression anchor is the demo company's value per share, `DEMO.MI`
`D83 = 11.4538271570406`: any change to the DCF routines, the Excel cell
formulas or the five-year fiscal window shows up as a departure from it.
...
<p align="center">
  <img src="screenshots/dashboard-up-to-date.png"
       alt="Dashboard, up to date"
       width="900">
</p>
