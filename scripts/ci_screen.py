...
def marked_portfolio(db_path: Path, out_dir: Path, *,
                     sync_run: bool = True) -> dict:
    """
    Mark the portfolio to current prices, after syncing it to the latest
    screen unless `sync_run` is False. The between-screens price update
    passes False: re-applying the same run would re-open, at the run's
    old price, any position the price update has since closed.
    """
    from cache import Cache
    ...
    with Portfolio(config.PORTFOLIO_PATH) as pf, Cache(db_path) as db:
        run_path = latest_run(out_dir) if sync_run else None
