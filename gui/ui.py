...
-----------------------------
The first version offered two buttons and a count of companies ready to
value, which raised the obvious question: where does that number come
from? Eight stages run before a valuation is possible and each goes stale
on its own schedule. Showing two of them and hiding six cannot explain why
the count is 170 rather than 400, and cannot warn that the credit spread
ladder is fourteen months old and about to fail every workbook.
...
# Every stage the dashboard can start on its own. Previously only prices
# and the screen had buttons, which left six of the eight steps reachable
# from the terminal alone -- and the first of those is the one that
# discovers how many companies exist at all. The rest still need the
...
# used to be hand-typed from Damodaran's page every January because
# nothing could fetch it reliably; now something can (fetch_ratings.py),
# but the two source pages are fragile enough -- one has an unrelated
# second table sitting right next to the one this project uses -- that
# it still needs a person to look at the diff before it is written, so
# it stays a panel rather than becoming a plain Update button.
...
def settings(steps: list[dict], keys: dict, engine: dict | None = None) -> str:
    ...
    engine here too -- previously that only reached a startup print() in
    the terminal, which the Zero Terminal Policy rules out as the only
    place to see it. Silent when one is found; nothing to tell a working
    setup it is working.
    """
    key_rows = "".join(f"""
  <div class="stage">
